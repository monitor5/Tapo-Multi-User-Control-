...
from app.services.permissions import get_user_allowed_plugs, can_user_access_plug
...
def get_user_model(
    ...
    """FastAPI dependency that simply forwards the authenticated `User` model."""
    return user
...
async def list_plugs(
    ...
    try:
        ...
        logger.info(f"사용자 '{user.username}'의 접근 가능한 플러그: {allowed_plugs}")
        
        for name, ip in pyp100_service.plugs.items():
            # 권한 체크: 접근 불가능한 플러그는 건너뜀
            if name not in allowed_plugs:
                logger.debug(f"플러그 '{name}' 접근 권한 없음, 건너뜀")
                continue
            # 1) 상태 조회
            try:
...
async def reserve_on(
    ...
    # 권한 체크
    if not can_user_access_plug(user, name, db):
        logger.warning(f"사용자 '{user.username}'가 권한 없는 플러그 '{name}' 접근 시도")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"플러그 '{name}'에 대한 접근 권한이 없습니다."
        )
...
async def plug_status(
    ...
    # 권한 체크
    if not can_user_access_plug(user, name, db):
        logger.warning(f"사용자 '{user.username}'가 권한 없는 플러그 '{name}' 상태 조회 시도")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"플러그 '{name}'에 대한 접근 권한이 없습니다."
        )
